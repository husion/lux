...
use lux::{
    getwld, getwlr, spd_to_ler, spd_to_ler_many, spd_to_power, spd_to_xyz, spd_to_xyz_many,
    Observer, PowerType, SpectralMatrix, Spectrum, SpectrumNormalization, WavelengthGrid,
};
...
fn current_rust_basics_match_luxpy() {
    ...
    assert_eq!(
        vec![xyzbar_1931.spectrum_count() + 1, xyzbar_1931.wavelength_count()],
    ...
    assert_vec_close(&spd_interp_flat, &parse_vec(&baselines["spd_interp"]), 1e-12);
    ...
    assert_vec_close(&normalize_max_flat, &parse_vec(&baselines["normalize_max"]), 1e-12);
    ...
    assert_vec_close(&normalize_area_flat, &parse_vec(&baselines["normalize_area"]), 1e-12);
    ...
    assert_vec_close(&normalize_ru_flat, &parse_vec(&baselines["normalize_ru"]), 1e-12);
    ...
    assert_vec_close(&normalize_pu_flat, &parse_vec(&baselines["normalize_pu"]), 1e-12);
    ...
    assert_vec_close(&normalize_qu_flat, &parse_vec(&baselines["normalize_qu"]), 1e-12);
    ...
    .unwrap();
    assert_vec_close(&xyz_1964, &parse_vec(&baselines["xyz_relative_1964_10"]), 1e-9);
    ...
    .unwrap();
    let xyz_many_flat: Vec<f64> = xyz_many.into_iter().flat_map(|xyz| xyz.into_iter()).collect();
    assert_vec_close(&xyz_many_flat, &parse_vec(&baselines["xyz_relative_many"]), 1e-9);
